...
# ------------------------------------------------------------
#  COMPONENT DEFINITIONS – REALISTIC VALUES FROM TABLE 4,
#  THEN SCALED FOR FAST SIMULATION
# ------------------------------------------------------------
components = {
    "Grid": {
        "dist": "exponential",
        "lambda": 5.7e-5 * SCALE,      # original 5.7e-5 /h
        "mu": 0.1 * SCALE,             # original μ = 0.1 /h
        "up": True
    },
    "PCC_Breaker": {
        "dist": "exponential",
        "lambda": 1.0e-6 * SCALE,      # original 1.0e-6 /h
        "mu": 0.5 * SCALE,             # original μ = 0.5 /h
        "up": True
    },
    "PCC_Panel": {
        "dist": "exponential",
        "lambda": 2.0e-6 * SCALE,      # original 2.0e-6 /h
        "mu": 0.25 * SCALE,            # original μ = 0.25 /h
        "up": True
    },
    "PV_Array": {
        "dist": "weibull",
        "shape": 2.5,                  # β unchanged
        "scale": 200000.0 / SCALE,     # original η = 200,000 h
        "mu": 0.05 * SCALE,            # original μ = 0.05 /h
        "up": True,
        "age": 0
    },
    "PV_Inverter": {
...
# ------------------------------------------------------------
#  HELPER FUNCTIONS (unchanged from microgrid_sim_11)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
#  SIMULATION LOOP (same as microgrid_sim_11)
# ------------------------------------------------------------
fault_log = []
state_log = []
...
print("\n" + "="*60)
print("Files written (scaled simulation):")
print("  state_data_with_override.csv")
print("  sensor_data.csv")
print("  fault_log.csv")
print(f"Scaling factor used: {SCALE}")
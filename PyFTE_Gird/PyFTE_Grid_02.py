...
# Ensure all discovered components have a column in state_data
missing = [c for c in components if c not in state_data.columns]
if missing:
    print(f"Warning: The following components are not in state_data: {missing}")
    # Keep only those that exist
    components = [c for c in components if c in state_data.columns]
...
# ----------------------------------------------------------------------
truth_table = state_data[components + [top_event]].copy()
...
def make_be_map(minimal_cut_sets):
    """Assign BE# labels to each unique component, sorted alphabetically."""
    all_components = sorted({comp for mcs in minimal_cut_sets for comp in mcs})
    be_map    = {comp: f"BE{i+1}" for i, comp in enumerate(all_components)}
    be_legend = {f"BE{i+1}": comp for i, comp in enumerate(all_components)}
...
def build_boolean_expression(minimal_cut_sets):
    """Return factored Boolean expression using BE# labels, plus a legend."""
    be_map, be_legend = make_be_map(minimal_cut_sets)
    terms = [frozenset(be_map[c] for c in mcs) for mcs in minimal_cut_sets]
    expr  = factor_sop(terms)
    legend_lines = [f"  {label} = {name}" for label, name in sorted(be_legend.items())]
    legend = "Basic Events (BE) Legend:\n" + "\n".join(legend_lines)
    return legend, expr
...
print("Minimal cut sets (component names):")
for mcs in minimal_cut_sets:
    print(sorted(mcs))

legend, expr = build_boolean_expression(minimal_cut_sets)
print(f"\n{legend}")
print(f"\nFactored Boolean expression:")
print(f"  {top_event} = {expr}")
...
for comp in components:
    ...
    if not events:
        # No events recorded – component always up (from state_data)
        initial_state = True
        up_complete = []
        down_complete = []
        ...
        initial_state = state_data.loc[start_time, comp] == 1

...
# ------------------------------------------------------------
#  COMPONENT DEFINITIONS - UPDATED WITH CSV PARAMETERS
# ------------------------------------------------------------
components = {
    "Grid": {
        "dist": "exponential",
        "lambda": 5.7e-5,           # λ = 5.7e-5 /h (≈0.5 outages/year)
        "mu": 0.1,                   # μ = 0.1 /h (mean 10 h)
        "up": True
    },
    "PCC_Breaker": {
        "dist": "exponential",
        "lambda": 1.0e-6,            # λ = 1.0e-6 /h
        "mu": 0.5,                    # μ = 0.5 /h (mean 2 h)
        "up": True
    },
    "PCC_Panel": {
        "dist": "exponential",
        "lambda": 2.0e-6,            # λ = 2.0e-6 /h
        "mu": 0.25,                   # μ = 0.25 /h (mean 4 h)
        "up": True
    },
    "PV_Array": {
        "dist": "weibull",
        "shape": 2.5,                 # β = 2.5
        "scale": 200000,               # η = 200,000 h
        "mu": 0.05,                    # μ = 0.05 /h (mean 20 h)
        "up": True,
        "age": 0                        # Track age for Weibull
    },
    "PV_Inverter": {
        "dist": "weibull",
        "shape": 2.2,                 # β = 2.2
        "scale": 60000,                # η = 60,000 h
        "mu": 0.083,                   # μ = 0.083 /h (mean 12 h)
        "up": True,
        "age": 0                        # Track age for Weibull
    },
    "Battery_Pack": {
        "dist": "weibull",
        "shape": 3.0,                 # β = 3.0
        "scale": 40000,                # η = 40,000 h
        "mu": 0.033,                   # μ = 0.033 /h (mean 30 h)
        "up": True,
        "age": 0                        # Track age for Weibull
    },
    "BMS": {
        "dist": "exponential",
        "lambda": 3.0e-6,            # λ = 3.0e-6 /h
        "mu": 0.125,                   # μ = 0.125 /h (mean 8 h)
        "up": True
    },
    "PCS": {
        "dist": "weibull",
...
# ------------------------------------------------------------
#  HELPER FUNCTIONS - UPDATED FOR WEIBULL
# ------------------------------------------------------------
def attempt_failure(comp):
    """
    Determine if component fails in this time step.
    Supports both exponential and Weibull distributions.
    """
    if comp.get("dist", "exponential") == "exponential":
        ...
        # Weibull model - probability depends on component age
        beta = comp["shape"]           # Shape parameter
        eta = comp["scale"]             # Scale parameter
        
        # Calculate failure probability for this time step
        # Using Weibull CDF: F(t) = 1 - exp(-(t/η)^β)
        t1 = comp["age"]                 # Age at start of interval
        t2 = comp["age"] + DT_HOURS      # Age at end of interval
        
        # Probability of failure during this interval
        # P(fail in [t1, t2] | survived to t1) = [F(t2) - F(t1)] / [1 - F(t1)]
        F_t1 = 1 - np.exp(-(t1/eta)**beta)
        F_t2 = 1 - np.exp(-(t2/eta)**beta)
        
        # Conditional probability of failure during this time step
        prob_failure = (F_t2 - F_t1) / (1 - F_t1 + 1e-10)  # Add small epsilon to avoid division by zero
        
        return np.random.rand() < prob_failure

def attempt_repair(comp):
    # All repairs remain exponential as per CSV
...
# ------------------------------------------------------------
#  SIMULATION LOOP - UPDATED FOR WEIBULL AGE TRACKING
# ------------------------------------------------------------
fault_log = []
state_log = []
...
for step in range(N_STEPS):
    ...
    # ---- Update component ages (only for operational Weibull components) ----
    ...
    for name, comp in components.items():
        if comp["up"] and attempt_failure(comp):
            comp["up"] = False
            # Reset age on failure (component is replaced/repaired to "as good as new")
            if comp.get("dist") == "weibull":
                comp["age"] = 0
            fault_log.append([current_time, name, "FAILURE"])
        elif not comp["up"] and attempt_repair(comp):
            comp["up"] = True
            # Reset age on repair (component is "as good as new")
            if comp.get("dist") == "weibull":
                comp["age"] = 0
...
print("Files written:")
print("="*60)
print("  state_data_with_override.csv \n  sensor_data.csv \n  fault_log.csv")
